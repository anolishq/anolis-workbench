"""Workbench smoke test for the extracted anolis-workbench repo."""
...
_ENV["ANOLIS_WORKBENCH_OPEN_BROWSER"] = "0"
...
def get(path):
    with urllib.request.urlopen(f"http://localhost:3010{path}") as r:
...
def delete(path):
    req = urllib.request.Request(f"http://localhost:3010{path}", method="DELETE")
    with urllib.request.urlopen(req) as r:
...
try:
    ...
    req = urllib.request.Request(
        "http://localhost:3010/api/projects/smoke-system",
    ...
    req = urllib.request.Request(
        "http://localhost:3010/api/projects/smoke-system/launch",
        data=b"{}",
        headers={"Content-Type": "application/json"},
        method="POST",
    )
    ...
    with urllib.request.urlopen("http://localhost:3010/js/composer/launch.js") as r:
        ct = r.headers.get("Content-Type")
        src = r.read().decode()
    assert "javascript" in ct
    assert "restoreRunningState" in src
    print("GET /js/composer/launch.js     OK")

    with urllib.request.urlopen("http://localhost:3010/js/composer/log-pane.js") as r:
        src = r.read().decode()
    assert "connect" in src
    print("GET /js/composer/log-pane.js   OK")

    with urllib.request.urlopen("http://localhost:3010/js/composer/health.js") as r:
        src = r.read().decode()
    assert "startPolling" in src
    print("GET /js/composer/health.js     OK")

    # Session persistence endpoint
    with urllib.request.urlopen("http://localhost:3010/js/app.js") as r:
        src = r.read().decode()
    assert "restoreRunningState" in src
