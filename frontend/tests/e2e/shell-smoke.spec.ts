...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/provider-schemas", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ schema_version: 1, providers: {} }),
    });
  });
  ...
  await page.route("**/api/templates", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify([{ id: "demo", meta: { name: "Demo" } }]),
    });
  });
  ...
  await page.route("**/v0/automation/tree", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ tree: '<BehaviorTree name="main" />' }),
    });
  });
