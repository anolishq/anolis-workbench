<script lang="ts">
  ...
  function set(obj: Record<string, any>, key: string, val: any): void { obj[key] = val; onChanged(); }
  function setRt(key: string, val: any): void { set(system.topology.runtime, key, val); }
...
<section class="form-section">
  ...
  <div class="form-group">
    ...
    <input type="number" min="1" max="65535" value={rt.http_port ?? ''}
      onchange={(e: any) => { const n = Number(e.target.value); if (!isNaN(n)) setRt('http_port', n); }} />
  </div>
  ...
  <div class="form-group">
    ...
    <textarea rows="3" placeholder="http://localhost:3000"
      ...
      oninput={(e: any) => setCorsOrigins(e.target.value)}></textarea>
  </div>
  ...
  <div class="form-group">
    ...
    <input type="number" min="500" max="30000" value={rt.shutdown_timeout_ms ?? ''}
      onchange={(e: any) => { const n = Number(e.target.value); if (!isNaN(n)) setRt('shutdown_timeout_ms', n); }} />
  </div>

  <div class="form-group">
    <label>Startup timeout (ms)</label>
    <input type="number" min="5000" max="300000" value={rt.startup_timeout_ms ?? ''}
      onchange={(e: any) => { const n = Number(e.target.value); if (!isNaN(n)) setRt('startup_timeout_ms', n); }} />
  </div>

  <div class="form-group">
    <label>Polling interval (ms)</label>
    <input type="number" min="100" max="10000" value={rt.polling_interval_ms ?? ''}
      onchange={(e: any) => { const n = Number(e.target.value); if (!isNaN(n)) setRt('polling_interval_ms', n); }} />
  </div>
  ...
  <div class="form-group form-group-inline">
    <label>
      <input type="checkbox" checked={rt.telemetry?.enabled ?? false}
        onchange={(e: any) => { system.topology.runtime.telemetry = system.topology.runtime.telemetry || {}; system.topology.runtime.telemetry.enabled = e.target.checked; onChanged(); }} />
      {' '}Telemetry enabled
    </label>
  </div>
  ...
  {#if rt.telemetry?.enabled}
    ...
    <div class="form-group">
      ...
      <input type="number" min="1" max="100000" value={rt.telemetry?.influxdb?.batch_size ?? ''}
        onchange={(e: any) => { const n = Number(e.target.value); if (!isNaN(n)) setInflux('batch_size', n); }} />
    </div>
    <div class="form-group">
      <label>Influx flush interval (ms)</label>
      <input type="number" min="1" max="600000" value={rt.telemetry?.influxdb?.flush_interval_ms ?? ''}
        onchange={(e: any) => { const n = Number(e.target.value); if (!isNaN(n)) setInflux('flush_interval_ms', n); }} />
    </div>
  {/if}
  ...
  <div class="form-group">
    ...
    <input type="text" spellcheck="false" style="font-family:monospace"
      ...
      oninput={(e: any) => { system.paths.runtime_executable = e.target.value; onChanged(); }} />
